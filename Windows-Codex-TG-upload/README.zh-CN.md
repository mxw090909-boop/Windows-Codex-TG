...
`tg-codex` 用于在聊天软件中远程调用本机 `codex` 会话，支持 Telegram、飞书（长连接）和微信。
...
- 支持先把 Telegram 图片/文件下载到当前工作目录，再继续当前会话
- Telegram、飞书、微信可单独启用，也可同时启用
...
- 渠道凭据（按需）
  ...
  - 微信: 先执行一次 `./run_wechat.sh login` 生成本地 bot token
...
export ALLOWED_TELEGRAM_USER_IDS="123456789"          # 建议设置；出于安全原因默认必填，多个用逗号分隔
export TG_REQUIRE_ALLOWLIST=1                          # 可选，默认 1；设为 0 才允许任意 Telegram 用户访问
export TELEGRAM_INSECURE_SKIP_VERIFY=0                 # 可选，默认 0；仅在临时排查 TLS 问题时设为 1
...
export ALLOWED_WECHAT_USER_IDS="xxx@im.wechat"     # 可选；登录后默认会自动允许扫码者自己的 user_id
export WECHAT_REQUIRE_ALLOWLIST=1                   # 可选，默认 1；设为 0 才允许任意微信用户访问
...
- 已有微信登录凭证：默认启动微信渠道
- 设置 `WECHAT_ENABLED=0`：显式关闭微信渠道
- 多组都满足时：对应渠道会同时启动

安全说明：

- 现在 `run.sh` 默认要求配置 `ALLOWED_TELEGRAM_USER_IDS`，除非你显式设置 `TG_REQUIRE_ALLOWLIST=0`
- `TELEGRAM_INSECURE_SKIP_VERIFY` 默认值为 `0`；除非临时调试 TLS 问题，否则不要改成 `1`
- 如果还没完成微信登录，`run.sh` 会明确提示先执行 `./run_wechat.sh login`
...
微信渠道使用腾讯新的扫码登录流程，登录成功后的 token 会保存在 `.runtime/wechat/` 下。
...
- 当前版本每个服务实例只支持一个已登录微信账号
- 开启白名单保护时，登录扫码者自己的 `user_id` 会默认自动加入允许列表
- v1 仅支持微信私聊文本：暂不支持图片、文件、语音、群聊和编辑式流式回复
- 微信沿用和 Telegram、飞书相同的会话命令
...
- `run.sh` 和 `run.ps1` 启动时都会自动探测本地环境；如果本地 Whisper 可用，会默认启用 Telegram 语音转写
...
Telegram 的图片和文件消息会先下载到当前会话工作目录下的 `.codex-tg-attachments/`，再继续交给 Codex 处理。
...
- `wechat_codex_service.py`：微信服务主程序
- `run_wechat.sh`：仅管理微信渠道的启动脚本
- `codex_common.py`：共享的本地 Codex / 会话 / 状态工具
