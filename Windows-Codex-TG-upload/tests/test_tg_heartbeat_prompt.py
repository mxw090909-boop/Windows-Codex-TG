...
from tg_codex_bot import HEARTBEAT_BANNED_PATTERNS, HEARTBEAT_SESSION_PROMPT, TgCodexService
...
class TelegramHeartbeatPromptTests(unittest.TestCase):
    ...
    def build_service(
        ...
        name: str,
        *,
        heartbeat_prompt: str = HEARTBEAT_SESSION_PROMPT,
    ...
    def test_first_heartbeat_prompt_mentions_context_and_time(self) -> None:
        service = self.build_service("heartbeat_first_prompt")
        ...
        self.assertIn("优先抓住最近聊过的一个小细节来开口", prompt)
        self.assertIn(f"“{HEARTBEAT_BANNED_PATTERNS[0]}”", prompt)
    ...
    def test_followup_heartbeat_prompt_can_choose_skip(self) -> None:
        service = self.build_service("heartbeat_followup_prompt")
    ...
    def test_force_heartbeat_prompt_never_asks_for_skip(self) -> None:
        service = self.build_service("heartbeat_force_prompt")
    ...
    def test_custom_heartbeat_prompt_override_is_used(self) -> None:
        service = self.build_service("heartbeat_custom_prompt", heartbeat_prompt="CUSTOM HEARTBEAT")
        ...
        self.assertIn("CUSTOM HEARTBEAT", prompt)
        self.assertNotIn(HEARTBEAT_SESSION_PROMPT, prompt)
